...
bottom_file_content = load_file_content(os.path.join(os.getcwd(),"CPU","memory","RAM_resultado.mif"))
...
def encrypt_text_no_SIMD():
    ...
    top_file_content = load_file_content(os.path.join(os.getcwd(),"CPU","memory","algorithm_cifrado_no_SIMD.s"))
    top_text.configure(state=tk.NORMAL)
    top_text.insert(tk.END, top_file_content)
    top_text.configure(state=tk.DISABLED)
    compiler('algorithm_cifrado_no_SIMD.s',"algorithm_cifrado_no_SIMD.mif")
    print("Texto no SIMD cifrado.")
def encrypt_text_SIMD():
    clear_text_and_stepping()
    top_file_content = load_file_content(os.path.join(os.getcwd(),"CPU","memory","algorithm_cifrado_SIMD.s"))
    top_text.configure(state=tk.NORMAL)
    top_text.insert(tk.END, top_file_content)
    top_text.configure(state=tk.DISABLED)
    compiler('algorithm_cifrado_SIMD.s',"algorithm_cifrado_SIMD.mif")
    print("Texto SIMD cifrado.")
def decrypt_text_no_SIMD():
    clear_text_and_stepping()
    top_file_content = load_file_content(os.path.join(os.getcwd(),"CPU","memory","algorithm_descifrado_no_SIMD.s"))
    top_text.configure(state=tk.NORMAL)
    top_text.insert(tk.END, top_file_content)
    top_text.configure(state=tk.DISABLED)
    compiler('algorithm_descifrado_no_SIMD.s',"algorithm_descifrado_no_SIMD.mif")
    print("Texto no SIMD descifrado.")
def decrypt_text_SIMD():
    clear_text_and_stepping()
    top_file_content = load_file_content(os.path.join(os.getcwd(),"CPU","memory","algorithm_descifrado_SIMD.s"))
    top_text.configure(state=tk.NORMAL)
    top_text.insert(tk.END, top_file_content)
    top_text.configure(state=tk.DISABLED)
    compiler('algorithm_descifrado_SIMD.s',"algorithm_descifrado_SIMD.mif")
    print("Texto SIMD descifrado.")
# Guardar Texto de cifrado/descrifrado
def save_text():
    text = entry_text.get()
    result_file_path = os.path.join(os.getcwd(), "CPU", "memory", "RAM_input.mif")
    directory = os.path.dirname(result_file_path)
    if not os.path.exists(directory):
        os.makedirs(directory)
